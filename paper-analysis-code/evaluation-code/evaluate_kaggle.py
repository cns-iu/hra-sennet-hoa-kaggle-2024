...
sub_dir = os.path.join(data_dir, 'k4_winning_team_submissions')
...
for team_id in range(5):
    # bring the prediction dataframe
    team_id += 1
    team_id = str(team_id)
    performance.setdefault(team_id, {})
    filename = f'submission-team-{team_id}.csv'
    ...
    for image_id in df_merge['image'].unique():
        ...
        if image_id in performance[team_id].keys():
            print(f'Already have for team {team_id}, image_id {image_id}...')
            continue
        else:
            performance[team_id][image_id]= {}
        start = time.time()
        ...
        try:
            ...
            performance[team_id][image_id]['nsd_0'] = nsd_0
        except:
            performance[team_id][image_id]['nsd_0'] = 0
        try:
            ...
            performance[team_id][image_id]['nsd_1'] = nsd_1
        except:
            performance[team_id][image_id]['nsd_0'] = 0
        print(performance[team_id][image_id])
        gc.collect()
        ### 2. cldice
        # 1. make 3d iamge
        w = df_real['width'].iloc[0]
        ...
        try:
            ...
            performance[team_id][image_id]['cldice'] = cldice
        except:
            performance[team_id][image_id]['cldice'] = 0
        print(performance[team_id][image_id])
        gc.collect()
        ### 3. monai's assd
        ...
        performance[team_id][image_id]['assd'] = assd.item()
        gc.collect()
        ### 4. monai's dice
        try:
            dice = compute_dice(pred, true)
            performance[team_id][image_id]['dice'] = dice.item()
        except:
            performance[team_id][image_id]['dice'] = 0
        print(performance[team_id][image_id])
        gc.collect()
        ### (optional) surface dice - marching cube
        try:
            nsd_mcube = compute_surface_dice_score(df_pred, df_real, device = 'cuda')
            performance[team_id][image_id]['nsd_mcube'] = nsd_mcube
        except:
            performance[team_id][image_id]['nsd_mcube'] = 0
        print(performance[team_id][image_id])
        gc.collect()
        ### (optional) surface dice - monai
        try:
            nsd_monai_0 = compute_surface_dice(pred, true, [0], True, use_subvoxels = False)
            performance[team_id][image_id]['nsd_monai_0'] = nsd_monai_0.item()
        except:
            performance[team_id][image_id]['nsd_monai_0'] = 0
        try:
            nsd_monai_0_subvoxel = compute_surface_dice(pred, true, [0], True, use_subvoxels = True)
            performance[team_id][image_id]['nsd_monai_0_subvoxel'] = nsd_monai_0_subvoxel.item()
        except:
            performance[team_id][image_id]['nsd_monai_0_subvoxel'] = 0
        print(performance[team_id][image_id])
        gc.collect()
        # collect time
        end = time.time()
        performance[team_id][image_id]['time_took'] = end - start
        # save performance
